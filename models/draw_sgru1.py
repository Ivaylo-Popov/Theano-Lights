...
class Draw_sgru1(ModelULBase):
    def __init__(self, data, hp):
        ...
        if hp.load_model and os.path.isfile(self.filename):
            ...
            with self.params:
                W1 = shared_normal((gates*n_t, n_x + n_h, n_h), scale=hp.init_scale)
                W11 = shared_normal((gates*n_t, n_h, n_h), scale=hp.init_scale)
                W4 = shared_normal((gates*n_t, n_zpt, n_h), scale=hp.init_scale)
                W44 = shared_normal((gates*n_t, n_h, n_h), scale=hp.init_scale)
                b1 = shared_zeros((gates*n_t, n_h,))
                b4 = shared_zeros((gates*n_t, n_h,))
        ...
        mu_encoder = T.zeros((x.shape[0], n_t, n_zpt))
        log_sigma_encoder = T.zeros((x.shape[0], n_t, n_zpt))
        log_qpz = 0
        out_log_qpz = 0
        ...
        for t in xrange(0, n_t):
            ...
            #mu_encoder = T.set_subtensor(mu_encoder[:,t,:], mu_encoder_t)
            #log_sigma_encoder = T.set_subtensor(log_sigma_encoder[:,t,:], log_sigma_encoder_t)
        
            log_qpz += -0.5* T.sum(1 + 2*log_sigma_encoder_t - mu_encoder_t**2 - T.exp(2*log_sigma_encoder_t))
            #out_log_qpz += 0.5* T.sum(1 + 2*log_sigma_encoder_t - mu_encoder_t**2 - T.exp(2*log_sigma_encoder_t), axis = 1)
        ...
        #row_cost = out_log_pxz + out_log_qpz
        #var_cost = T.sum((row_cost - T.mean(row_cost))**2)
