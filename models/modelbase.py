...
class ModelLMBase(ModelBase):
    ...
    def compile(self, cost, te_cost, h_updates, te_h_updates, spx, add_updates=[]):
        seq_idx = T.iscalar()
        learning_rate = T.fscalar()
        offset = T.iscalar()
...
class ModelMPBase(ModelBase):
    def __init__(self, id, data, hp):
        ...
        self.X.tag.test_value = np.random.randn(hp.seq_size, hp.batch_size, data['n_x']).astype(dtype=theano.config.floatX)
        self.Y.tag.test_value = np.random.randn(hp.seq_size, hp.batch_size, data['n_y']).astype(dtype=theano.config.floatX)

        self.data = copy.copy(data)
        for key in ('tr_X', 'va_X', 'te_X', 'tr_Y', 'va_Y', 'te_Y'):
            if key in self.data:
                self.data['len_'+key] = (self.data[key].shape[0], self.data[key].shape[1])
                self.data[key] = shared(self.data[key], borrow=True)
    ...
    def train_epoch(self, it_lr, offset=0):
        tr_outputs = None
        num_batches = self.data['len_tr_X'][0] * ((self.data['len_tr_X'][1] - offset) / self.hp.seq_size)
        seq_per_epoch = self.hp.batch_size * (self.hp.seq_size - self.hp.warmup_size) * num_batches

        self.reset_hiddenstates()
        
        for i in xrange(0, num_batches):
            group_idx = i / ((self.data['len_tr_X'][1] - offset) / self.hp.seq_size)
            seq_idx = i % ((self.data['len_tr_X'][1] - offset) / self.hp.seq_size)
            outputs = self.train(group_idx, seq_idx, it_lr, offset)
            outputs = map(lambda x: x / float(seq_per_epoch), outputs)
            if i==0:
                tr_outputs = outputs
            else:
    ...
    def test_epoch(self):
        te_outputs = None
        num_batches = self.data['len_te_X'][0] * (self.data['len_te_X'][1] / self.hp.seq_size)
        seq_per_epoch = self.hp.batch_size * (self.hp.seq_size - self.hp.warmup_size) * num_batches

        self.reset_hiddenstates()

        for i in xrange(0, num_batches):
            group_idx = i / (self.data['len_te_X'][1] / self.hp.seq_size)
            seq_idx = i % (self.data['len_te_X'][1] / self.hp.seq_size)
            outputs = self.test(group_idx, seq_idx)
            outputs = map(lambda x: x / float(seq_per_epoch), outputs)
            if te_outputs is None:
                te_outputs = outputs
            else:
    ...
    def dyn_validation_epoch(self, it_lr):
        ...
        for i in xrange(0, num_batches):
            ...
            outputs = self.dyn_validate(group_idx, seq_idx, it_lr)
            outputs = map(lambda x: x / float(seq_per_epoch), outputs)
            if te_outputs is None:
                te_outputs = outputs
            else:
    ...
    def dyn_test_epoch(self, it_lr):
        ...
        for i in xrange(0, num_batches):
            group_idx = i / (self.data['len_te_X'][1] / self.hp.seq_size)
            seq_idx = i % (self.data['len_te_X'][1] / self.hp.seq_size)
            outputs = self.dyn_test(group_idx, seq_idx, it_lr)
            outputs = map(lambda x: x / float(seq_per_epoch), outputs)
            if te_outputs is None:
                te_outputs = outputs
            else:
    ...
    def compile(self, cost_grad, cost, te_cost, h_updates, te_h_updates, cost_base, te_cost_base, info, te_info, add_updates=[], exposure=[]):
        ...
        self.train = theano.function(inputs=[group_idx, seq_idx, learning_rate, offset], updates=updates + h_updates,
                                     givens={
                                         self.X:self.data['tr_X'][group_idx, 
